# AWS one-click example

This example is a wrapper root module that:
1. Calls the main module to allocate a VPC CIDR + subnets in BloxOne DDI (IPAM)
2. Creates an AWS VPC and Subnets using those CIDRs
3. Optionally creates demo EC2 instances with static IPs allocated from BloxOne
...
- You must configure authentication for both providers:
  ...
  - `aws` (AWS CLI credentials, env vars, IAM role, etc.)
...
Set `aws_vm_enabled = true` in `demo.auto.tfvars` to provision EC2 instances with static private IPs
...
If `aws_vm_ssh_public_key` is not set, Terraform generates an ED25519 keypair. Retrieve the
private key after apply with:
```
terraform -chdir=examples/AWS/one-click output -raw aws_vm_private_key_pem
```

If `aws_vm_public_ip = true`, an Internet Gateway and public route table are also created so the
instances are reachable from the internet via their auto-assigned public IPs.
